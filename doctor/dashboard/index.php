                ...
                <div class="container-fluid">
                    ...
                    <section class="myDivisions">
                        <div class="split-managements">
                            <?php
                                // Selecting patients under specific doctor
                                $selDoc = "SELECT id from doctors WHERE DocName='{$_SESSION['doc_name']}'";
                                $ret = mysqli_query($mysqli, $selDoc);
                                $docRow = mysqli_fetch_assoc($ret);
                                $docid = $docRow['id'];

                                // selecting patients
                                $selPat = "SELECT * FROM patients WHERE Doc_id=$docid";
                                $return = mysqli_query($mysqli, $selPat);
                                $cntPat = mysqli_num_rows($return);

                                // select appointments
                                $selApp = "SELECT * FROM appointments WHERE doc_id=$docid";
                                $res = mysqli_query($mysqli, $selApp);
                                $cntApp = mysqli_num_rows($res);
                            ?>
                            <div class="my-card">
